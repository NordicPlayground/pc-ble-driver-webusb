const { NRF_SUCCESS, NRF_ERROR_INVALID_PARAM, sd_rpc_app_status_t } = require('../../../../src/js/sd_rpc_types');
const { ble_gap_scan_params_t, ble_gap_conn_params_t, ble_gap_addr_t, ble_uuid_t, ble_gattc_handle_range_t } = require('../../../../src/js/bindings/bleSDAttributeStructs');
const { sd_ble_gap_connect, sd_ble_gap_conn_param_update } = require('../../../../src/js/ble_impl/ble_gap_impl');
const { sd_ble_gattc_primary_services_discover, sd_ble_gattc_characteristics_discover, sd_ble_gattc_descriptors_discover, sd_ble_gattc_exchange_mtu_request } = require('../../../../src/js/ble_impl/ble_gattc_impl');
const { uuidToCharacteristic } = require('../../../../src/js/gattc');
...
class BleTransport extends EventEmitter {
    constructor(transportParameters, bleCfgTag) {
        ...
        addEventListener('connParamUpdateRequest', this._handleConnParamUpdateRequest);
        addEventListener('onDisconnected', this.onDisconnected);
    ...
    async init() {
        ...
        return this.addVsUuid()
            ...
            .then(() => {
                const connUpdatedPromise = new Promise(resolve => addEventListener('connParametersUpdated', () => resolve()));
                const secParamsUpdatedPromise = new Promise(resolve => addEventListener('secParametersUpdated', () => resolve()));
    ...
    async _connect(targetAddress, targetAddressType) {
        ...
        const onConnectedPromise = new Promise((resolve, reject) => {
            const evtCb = (evt) => {
                removeEventListener('onConnected', evtCb)
            ...
            addEventListener('onConnected', evtCb)
        });
        ...
        reject(`Could not connect to device. Error ${apiRes}`);
    }
    ...
    destroy() {
        ...
        removeEventListener('connParamUpdateRequest', this._handleConnParamUpdateRequest);
        this.cleanup()
    }
    ...
    _setMtuSize(mtuSize) {
        return new Promise((resolve, reject) => {
            const mtuRequestCb = evt => {
                removeEventListener('mtuExchangeResponse', mtuRequestCb);
            ...
            addEventListener('mtuExchangeResponse', mtuRequestCb);
        ...
        return new Promise((resolve, reject) => {
            ...
            const disconnectionHandler = evt => {
                if (!this.connected || (evt && evt.detail && evt.detail.handle === this.handle)) {
                    ...
                    removeEventListener('onDisconnected', disconnectionHandler);
                    resolve();
                }
            };
            ...
            addEventListener('onDisconnected', disconnectionHandler);
        });
    }
    ...
    async _handleConnParamUpdateRequest(evt) {
        ...
        connParams.delete();
        dispatchEvent(new Event('connParametersUpdated'));
    }
    ...
    async serviceDiscoveryStart() {
        ...
        const foundDfuServicePromise = new Promise((resolve, reject) => {
            ...
            const evtCb = (evt) => {
                if (!evt || !evt.detail) {
                    removeEventListener('serviceDiscovered', evtCb);
            ...
            addEventListener('serviceDiscovered', evtCb)
        });
        ...
        const discoveredCharPromise = new Promise((resolve, reject) => {
            const discoveredCb = evt => {
                if (evt.detail.characteristic === undefined) {
                    removeEventListener('characteristicDiscovered', discoveredCb);
                    resolve(gattcService);
                }
            };
            addEventListener('characteristicDiscovered', discoveredCb);
        });
        ...
        const discoveredDescriptorsPromise = new Promise((resolve, reject) => {
            const discoveredCb = evt => {
                if (evt.detail.descriptor === undefined) {
                    removeEventListener('descriptorDiscovered', discoveredCb);
                    resolve(gattcService);
                }
                console.log("Added descriptor")
            };
            addEventListener('descriptorDiscovered', discoveredCb);
        });
